...
if(isset($_POST['phone_no_button'])){
    ...
    if (update_user_phone_no($phone_no,$user_id)){
        if ($_SESSION['edit_phone_source'] == "profile.php") {
            header("Location: profile.php?id=" . $user_id);
            } else {
                header("Location: checkout.php");
            }
    } else{
        $error_message = "Sorry, your phone number could not be updated";
    }
}
...
echo "<br>";
  $current_phone_no = "0" . $show_user_details['phone_no'];
  echo "<form method='post'>";
  echo "<label for='phone_no'>Phone No</label>";
  echo "<input id='phone_no' type='number' name='phone_no' value='$current_phone_no'><br>"; 
  echo "<input type='submit' id='phone_no_button' name='phone_no_button' value='Update Phone Number' class='button'>";
  echo "</form>";
